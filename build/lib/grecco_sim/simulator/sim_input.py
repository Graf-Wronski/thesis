...
class DummyInputDataLoader(InputDataLoader):
    ...
    def _get_load(self):
        ...
        diff = np.array(
            [
                (
                    datetime.datetime.combine(noon.date(), idx.time()) - noon
                ).total_seconds()
                / 3600.0
    ...
    def get_parameters(self, sys_id: str) -> type_defs.SysPars:
        assert (
            sys_id in self.sys_ids
        ), f"Requested sys_id '{sys_id}' not in available Ids"

        if sys_id == "dummy_00":
            return type_defs.SysParsPVBat(
                sys_id,
                c_sup=0.3,
                c_feed=0.1,
                eff=0.9,
...
class SampleInputDataLoader(InputDataLoader):
    ...
    def _load_data(self):
        if not os.path.exists(self.data_path):
            raise ValueError(
                f"Given data path for scenario input {self.data_path} is non-existant"
            )
        ...
        if self.scenario["focus"] == "hp":
            ...
            self._data["weather"] = data.reset_index(drop=True).set_index(
                self.time_index
            )
    ...
    def get_input_data(self, sys_id: str, scenario: Dict) -> pd.DataFrame:
        data = {f"{sys_id}_load_p_load": self._data["load"][sys_id].values}
        ...
        if self.scenario["focus"] == "hp":
            data.update(
                {
                    _col: self._data["weather"][_col].values
                    for _col in self._data["weather"]
                }
            )
...
class PyPsaGridInputLoader(InputDataLoader):
    ...
    def __init__(
        ...
        if "grid_data_path" and "weather_data_path" in scenario:
            ...
            weather_data_path = (
                path.parent.parent / "data" / "opfingen" / "weather_data.csv"
            )

        self.weather_data = pd.read_csv(
            weather_data_path, index_col=0, date_format="%Y-%m-%d %H:%M:%S"
        )
    ...
    def get_input_data(self, sys_id: str, scenario: Dict) -> pd.DataFrame:
        ...
        if sys_id in self.grid.get_pv_ts():
            data[f"{sys_id}_pv_p_ac"] = self.grid.get_pv_ts().loc[
                self.time_index, sys_id
            ]
    ...
    def get_parameters(self, sys_id: str) -> type_defs.SysPars:
        ...
        if sys_id in self.grid.pv_p:
            ...
            initial_temp = (
                type_defs.SysParsHeatPump.temp_min_heat
                + self.grid.heat_pumps.index.get_loc(sys_id)
                / len(self.grid.heat_pumps.index)
                * (
                    type_defs.SysParsHeatPump.temp_max_heat
                    - type_defs.SysParsHeatPump.temp_min_heat
                )
            )
