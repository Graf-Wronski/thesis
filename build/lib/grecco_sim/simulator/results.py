...
class SimulationResult(object):
    ...
    def __init__(self) -> None:
        ...
        # Parameterization
        self._sizing: dict[str, type_defs.SysParsPVBat]| None = None
    ...
    def from_simulation(self, run_params, raw_output, assigned_grid_fees, sizing, time_index, exec_time):
        ...
        self._agents_ts = {sys_id: generate_agent_ts(raw_output[sys_id], time_index) for sys_id in raw_output}
    ...
    def from_files(self, directory_name: pathlib.Path | str):
        ...
        with open(directory_name / "run_params.json") as run_params:
            _run_params = json.load(run_params)
            self._run_params = type_defs.RunParameters(**_run_params)

        self._sizing = data_io.load_sizing(directory_name / "system_parameters.json")

        self._assigned_grid_fees = data_io.read_ts(directory_name / f"signals_costs_{self.run_params.sim_tag}.csv")

        self._ts_grid = data_io.read_ts(directory_name/ f"results_{self.run_params.sim_tag}.csv")
    ...
    def record(self):
        ...
        data_io.write_ts(self.ts_grid, out_dir / f"results_{sim_tag}.csv")
        ...
        data_io.dump_parameterization(out_dir / "run_params.json", dataclasses.asdict(self.run_params))
...
def generate_agent_ts(agent_output, time_index):
    ...
    individual_dfs = [
        pd.DataFrame(
            data={key: agent_output[key] for key in ["grid", "c_supp", "c_feed"]}
        )
    ]
    ...
    if "bat" in agent_output:
        individual_dfs += [
            pd.DataFrame(
                data={f"bat_{key}": agent_output["bat"][key][:-1] for key in ["soc"]}
            ),
            pd.DataFrame(
                data={
                    f"bat_{key}": agent_output["bat"][key] for key in ["p_ac", "p_net"]
                }
            ),
        ]
    ...
    if "hp" in agent_output:
        individual_dfs += [
            pd.DataFrame(
                data={f"hp_{key}": agent_output["hp"][key][:-1] for key in ["temp"]}
            ),
            pd.DataFrame(
                data={
                    f"hp_{key}": agent_output["hp"][key] for key in ["p_in", "q_hp", "u_ext"]}
            )
        ]
