...
from grecco_sim.models import grid_node
...
class SimulationSetup(object):
    ...
    def __init__(self, run_parameters: RunParameters):
        ...
        self.time_index = pd.date_range(
            start=self.run_params.start_time, freq=self.run_params.dt, periods=self.run_params.sim_horizon
        )
        self.sys_ids = []
    ...
    def _get_nodes(self, opt_pars: type_defs.OptParameters):
        ...
        elif self.run_params.scenario["name"] == "opfingen":
            dl = sim_input.PyPsaGridInputLoader(self.time_index, self.run_params.scenario)
        else:
    ...
    def run_sim(
        ...
        sim_result = sim_manager.get_sim_result()

        # Write sim results to disc
        sim_result.record()

        # Make evaluation
        kpis = simulation_eval.evaluate_sim(sim_result)
