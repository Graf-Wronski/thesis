...
class FirstOrderCoordinatorBase(coord_interface.CoordinatorInterface):
    ...
    def get_cost_realization(
        ...
        """
        assert (
            set(initial_futures.keys())
            == set(realization_grid.keys())
            == set(signals.keys())
        )
...
class CoordinatorGradientDescent(FirstOrderCoordinatorBase):
    ...
    def _update_lambda(
        self, y_ks: Dict[str, np.ndarray], lam: np.ndarray
    ) -> np.ndarray:
        """Return Updated lambda for gradient descent."""
        return lam + self.opt_p.alpha * np.array(
            [y_ks[a] for a in y_ks]
        ).sum(axis=0)
...
class CoordinatorVujanic(coord_interface.CoordinatorInterface):
    ...
    def _init_iteration(self, k, current_horizon, sys_ids):
        ...
        self.s_upper = {sys_id: -self.MAX_CONSTR_VAL * np.ones(current_horizon) for sys_id in sys_ids}
        self.s_lower = {sys_id: self.MAX_CONSTR_VAL * np.ones(current_horizon) for sys_id in sys_ids}
    ...
    def get_signals(self, futures):
        ...
        for sys_id in sys_ids:
            ...
            # Update Lambda
            new_lam[sys_id] = np.maximum(0., prev_lam + 1. / self.alpha_inv * (grid_lim - self.p_lim + rho_all))
