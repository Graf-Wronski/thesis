...
class LocalControllerBase(abc.ABC):
    ...
    @abc.abstractmethod
    def get_control(self, signal: sig_types.SignalType, state: dict[str, Any]) -> float:
...
class LocalControllerHeatPumpOnOff(LocalControllerBase):
    def __init__(self, model_params: type_defs.SysParsHeatPump):
        super().__init__()
...
class LocalControllerPassControl(LocalControllerBase):
    ...
    def get_control(self, signal: sig_types.DirectControlSignal, state):
    ...
    def get_flex_schedule(
        ...
        future = type_defs.LocalFuture(forecast.fc_res_load, flex_type="continuous")
        return future
