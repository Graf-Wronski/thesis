...
def dot(x: Union[casadi.SX, np.ndarray], y: Union[casadi.SX, np.ndarray]) -> casadi.SX:
...
A_VERY_HIGH_NUMBER = np.infty


# ============= Modeling functions (for state evolution) ==================
def function_x_next_ev_on_off(p_nom_ch, dt_h, capacity, eff):
    x = casadi.SX.sym("x", 1)
    u = casadi.SX.sym("u", 1)
...
class MyNLPSolver:
    ...
    def __init__(
        ...
        solver="ipopt",
        user_functions: Dict[str, casadi.SX] | None = None,
    ):
    ...
    def _init_solver(self, obj, states, constraints: List[MyConstr], parameters, user_functions: Dict[str, casadi.SX] | None):
        ...
        # discrete = casadi.vertcat(*[casadi.SX(x.discrete) for x in states])
        discrete = np.concatenate([np.array([x.discrete] * x.horizon) for x in states])
        discrete = list(discrete)

        g = casadi.vertcat(*[g.expr for g in constraints])
        p = casadi.vertcat(*[parameters[par_name].sx for par_name in parameters])
        ...
        # print("Using solver", self.solver_name, "for problem.")
        ...
        self.state_toc = {}
        for x in states:
            self.state_toc[x.name] = (_idx_start, _idx_start + x.horizon)
            _idx_start += x.horizon
    ...
    def solve(self, parameter_values, init_guess=None):
        ...
        if init_guess is None:
            ...
            assert len(init_guess) == len(self.state_dim)
            _init_guess = casadi.vertcat(*init_guess)

        self.sol = self._pure_casadi_solve(_init_guess, _parameters)
