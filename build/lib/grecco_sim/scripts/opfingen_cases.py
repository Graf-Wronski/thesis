...
# This pypsa folder contains all time steps in which congestion and voltage issues are expected. Results come from indigo simulations
PATH_WITH_REDUCED_TS = (pathlib.Path(__file__).parent.parent.parent
...
SELECTED_CASES = [datetime.datetime(2023, 1, 1, 0, 0, 0, tzinfo=pytz.utc),
...
def select_congestion_cases(number_of_cases: int, season="", path=""):
    ...
    if season == 'winter':
        snapshots_filtered = [date for date in snapshots
                              if parser.parse(date).month in [1, 11, 12]]  # November, December, January
    ...
    congestion_dates = [parser.parse(date).replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
                        for date in congestion_cases]  # Start anaylsis from midnight
...
def run_sim(run_name: str, congestion_date):
    ...
    coordination_mechanisms = ["none", "local_self_suff"]
