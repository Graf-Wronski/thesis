...
from grecco_sim.controller import problem_combiner
...
def plot(parameters, lam, grid_combined, y_ks, u_ks, res_power_set=None, grid_plus_slack=None, lam_slack=None,
         show=True):
    ...
    if grid_plus_slack is not None:
        ax[3].plot(parameters.times, grid_plus_slack, label="grid+slack (where != limit)", drawstyle="steps-post")
    ax[3].plot(parameters.times, [parameters.p_lim] * len(parameters.times), label="Limit", drawstyle="steps-post")
    ax[3].fill_between(parameters.times, [parameters.p_lim] * len(parameters.times), grid_combined,
                       where=grid_combined > parameters.p_lim, step="post",
                       alpha=0.2, color="red", label="Constraint")

    for a in y_ks:
        ax[a].plot(parameters.times, y_ks[a], linestyle="dotted", drawstyle="steps-post",
                   label=f"Grid "r"($x^s_a - x^f_{a}$)")
        fac_bat_pow = parameters.sim_pars[a]["p_inv"] if parameters.on_off[a] else 1.
        ax[a].plot(parameters.times, fac_bat_pow * u_ks[a], linestyle="dashed", drawstyle="steps-post", label=f"Control {a}")
        if res_power_set is not None:
            ax[a].plot(parameters.times, res_power_set[a], linestyle="dashdot", drawstyle="steps-post", label="set")
...
class Parameters:
    ...
    sim_pars = [
        dict(c_sup=[0.2 + 0.0005 * k for k in times], c_feed=0., capacity=30., dt_h=1., p_inv=5., name="ag0"),  # Agent 0
        dict(c_sup=[0.41 + 0.0005 * k for k in times], c_feed=0., capacity=30., dt_h=1., p_inv=5., name="ag1"),  # Agent 1
    ]
    on_off = [False, False]
...
def centralized(parameters: Parameters):
    ...
    for i in range(parameters.n_agents):
        ...
        if parameters.on_off[i]:
            agents[f"ag_{i:02d}"] = local_solver_pv_bat.OnOffPVBatSolver(parameters.horizon, f"ag_{i:02d}", _pars)
        else:
            agents[f"ag_{i:02d}"] = local_solver_pv_bat.CasadiSolverLocalPVBat(parameters.horizon, f"ag_{i:02d}", _pars, {"solver": "gurobi"})

    combiner.combine(agents, parameters.p_lim)

    par_values = {f"ag_{i:02d}": {
        "x_init": [parameters.soc_init],
        "lam_a": [0.] * parameters.horizon,
        "fc_p_unc": parameters.fc_res_load[i]} for i in range(parameters.n_agents)
    }
...
def vujanic(parameters: Parameters):
    ...
    s_up = [np.array([-100000.] * parameters.n_constr)] * parameters.n_agents
    ...
    for iteration in range(parameters.max_iter):
        ...
        # for a in [iteration % n_agents]:
            ...
            if parameters.on_off[a]:
                agent = local_solver_pv_bat.OnOffPVBatSolver(parameters.horizon, f"ag_{a:02d}", _pars)
            else:
                agent = local_solver_pv_bat.CasadiSolverLocalPVBat(parameters.horizon, f"ag_{a:02d}", _pars, {"solver": "gurobi"})
        ...
        lam = np.maximum(0., lam + alpha(iteration) * (grid_combined - parameters.p_lim))
        ...
        if iteration == parameters.max_iter - 1:
            plot(parameters, {i: lam for i in range(parameters.n_agents)}, grid_combined, y_ks, u_ks, show=False)

    fig, ax = style.styled_plot(ylabel="Summed Constraint violations", xlabel="Iterations", figsize="landscape")
...
def single_admm(parameters: Parameters):
    ...
    g_fig, g_ax = style.styled_plot(title="ADMM schedule evolution", xlabel="time", ylabel="power", figsize="landscape")
    ...
    for iteration in range(parameters.max_iter):
        ...
        for a in range(parameters.n_agents):
            ...
            agent = local_admm.LocalADMMSolver(parameters.horizon, a, _pars, {"rho": parameters.rho, "solver": "gurobi"})
        ...
        coord = coord_admm.ADMMCentralSolver(parameters.horizon, sys_ids,
                                                   parameters.rho, parameters.p_lim)
        # For the slack agent
        yk_slack = coord.handle_slack_local_problem({"lambda": lam_slack, "res_power_set": ref_power_grid_slack})
        ...
        if iteration == parameters.max_iter - 1:
            plot(parameters, lam, grid_combined, y_ks, u_ks, res_power_set=res_power_set,
                 grid_plus_slack=grid_plus_slack, lam_slack=lam_slack, show=False)
            g_ax.plot(parameters.times, y_ks[0], label=f"Iteration {iteration}, agent 0", drawstyle="steps-post")
            g_ax.plot(parameters.times, y_ks[1] + iteration * 0.1 / 400., label=f"Iteration {iteration}, agent 1",
