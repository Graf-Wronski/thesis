...
def plot(
    ...
    if lam is not None:
        for a in lam:
            ax[2].plot(
                parameters.times, lam[a], drawstyle="steps-post"  # label=f"lam[{a}]",
            )
    ...
    for a, sys_id in enumerate(y_ks):
        ...
        fac_bat_pow = (
            parameters.sim_pars[sys_id]["p_inv"] if parameters.on_off[sys_id] else 1.0
        )
        ax[a].plot(
            parameters.times,
...
class Parameters:
    ...
    # 1 for no plots
    plot_every = 199
    opt_pars = type_defs.OptParameters(solver_name= "osqp", rho = 1000.0, mu=100.0, horizon=horizon, alpha=1.)
    ...
    def __init__(self, problem_descr: dict = {}) -> None:
        ...
        if problem_descr:
            ...
            self.on_off = {
                sys_id: self.sim_pars[sys_id].on_off for sys_id in self.sim_pars
            }
...
def centralized(parameters: Parameters):
    ...
    for sys_id in parameters.sys_ids:
        ...
        if parameters.on_off[sys_id]:
            ...
            agents[sys_id] = local_solver_pv_bat.OnOffPVBatSolver(
                parameters.horizon, sys_id, _pars
            )
        else:
...
def single_admm(parameters: Parameters):
    ...
    coord = coord_admm.CoordinatorADMM(
        parameters.opt_pars,
        parameters.sys_ids,
        type_defs.GridDescription(parameters.p_lim)
    )
    ...
    current_signals = coord.get_signals({a: type_defs.LocalFuture(
        yg=y_ks[a],
        _meta={"fc": u_ks[a], "state": {"k": 1}}
        ) for a in y_ks})
    ...
    for iteration in range(parameters.max_iter):
        ...
        current_signals = coord.get_signals({
            a: type_defs.LocalFuture(
                yg=y_ks[a], _meta={"fc": u_ks[a], "state": {"k": 1}}
            ) for a in y_ks
        })
...
def single_second_order(parameters: Parameters):
    ...
    grid_combined = np.sum(np.array([np.array(y_ks[a]) for a in y_ks]), axis=0)
    grid_plus_slack = grid_combined
    plot(parameters, {a: coord.lam for a in y_ks},
                grid_combined,
                y_ks,
                u_ks,
                coord.ref_power_grid,
    ...
    set_viols = [schedule_eval.ScheduleEvaluator(yg_set, parameters.sim_pars).get_constr_viol(parameters.p_lim)]
    lams = [coord.lam.sum()]
...
def compare(parameters: Parameters):
    ...
    plot_meta(c_admm, v_admm, c_vuja, v_vuja, c_cent, v_cent, c_sec_ord, v_sec_ord,
...
if __name__ == "__main__":
    ...
    inspection_file_name = "/home/agross/src/python/grecco/grecco_sim/results/default/240906_1100/inspection.json"
    ...
    for a in pars.sim_pars:
        pars.sim_pars[a].init_soc = 0.

    # pars.p_lim=4.2
    pars.opt_pars.mu = 500.
