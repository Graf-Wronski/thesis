...
class ThermalSystem(model_base.Model):
    ...
    def __init__(self, sys_id: str, horizon: int, dt_h: float,
                 params: type_defs.SysParsHeatPump, weather_info: pd.DataFrame):
        """
        ...
        self.temp_out = np.array(weather_info["Outside Temperature"])
        self.irradiation = np.array(weather_info["Solar Irradiation"])
    ...
    def _init_model(self, params: type_defs.SysParsHeatPump):
        ...
        hp_db_path = hp_db_path / "data" / "heat_pump_database" / "heat_pump_database_short_version.csv"
        ...
        if self.heat_pump_type is not None:  # Assigning a model if known
            ...
            if self.heat_pump_model == "on-off":
                ...
                raise ValueError(f"ERROR: Entered model {self.heat_pump_model} is not valid. Select from 'on-off' or 'variable-speed'")
        else:
    ...
    def _evolve(self, control: float):
        ...
        heat_from_solar_irradiation = self.irradiation[now] * self.absorbance * self.irradiance_area / 1000  # In kW
        net_heat = heat_transfer_to_outside + self.heat_pump.q_out + heat_from_solar_irradiation

        self.temp[after] = self.temp[now] + ((1 / self.params.thermal_mass) * net_heat * 3600 * self.dt_h)
