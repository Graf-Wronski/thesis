...
class CoordinatorSecondOrder(coordinator.Coordinator):
    """
    ...
    def __init_iteration(self, flex_futures: Dict[str, type_defs.Schedule], inflex_sum: np.ndarray):
        self.lam = np.zeros(list(flex_futures.values())[0].horizon)
        self.ref_power_grid = {a: flex_futures[a].yg for a in flex_futures}

        self.ref_power_grid[self.SLACK_AGENT] = (
            -np.array([flex_futures[a].yg for a in flex_futures]).sum(axis=0)
        )

        self.constraint_value = 1e10
        self.prev_constraint_value = 2 * self.constraint_value

    def get_signal(self, sys_id) -> Union[signal.SecondOrderSignal, signals.FirstOrderSignal]:
        ...
        if sys_id in self.ref_power_grid:
            return signal.SecondOrderSignal(
                mul_lambda=self.lam, res_power_set=self.ref_power_grid[sys_id]
            )
        else:
            return signals.FirstOrderSignal(self.lam)

    def get_signals(
        self, futures: Dict[str, type_defs.Schedule]
        ...
        if self.current_time != list(futures.values())[0].t:
            self.__init_iteration(flex_futures, inflex_sum)
            self.current_time = list(futures.values())[0].t


        # Solve 'local' problem for the slack agent
        yk_static = handle_static_agent_problem(
            signal.SecondOrderSignal(mul_lambda=self.lam, res_power_set=self.ref_power_grid[self.SLACK_AGENT]),
            self.opt_p.rho,
            self.grid_p.p_lim,
            inflex_sum,
        ...
        y_ks = {a: flex_futures[a].p_grid for a in flex_futures}
        y_ks[self.SLACK_AGENT] = yk_static
        self.constraint_value = np.abs(np.array([y_ks[a] for a in y_ks]).sum(axis=0)).sum() / self.horizon
        ...
        active_constraints[self.SLACK_AGENT] = _get_active_constraints_static_agent(yk_static, inflex_sum, self.grid_p.p_lim)
    ...
    def get_grid_fees(
        self,
        initial_futures: Dict[str, type_defs.Schedule],
        realization_grid: Dict[str, float],
        signals: Dict[str, signal.SecondOrderSignal],
    ) -> Dict[str, float]:
        assert set(initial_futures.keys()) == set(realization_grid.keys()) == set(signals.keys())

        ret = {f"fee_{sys_id}": signal.mul_lambda[0] * realization_grid[sys_id] for sys_id, signal in signals.items()}
...
def _create_second_order_dual_problem(horizon, sys_ids, mu, qp_solver_name, active_constraints):
    ...
    qp_slack = mycas.MySX("qp_slack", -np.infty, np.infty, horizon=horizon)
    states += [qp_slack]
    ...
    for sys_id in sys_ids:
        ...
        d_pg_ref_a = mycas.MySX(f"d_pg_ref_a_{sys_id}", -np.infty, np.infty, horizon=horizon)
        states += [d_pg_ref_a]
        ...
        for jac in active_constraints[sys_id]:
            ...
            constraints += [mycas.MyConstr(mycas.dot(d_pg_ref_a.sx, np.array(jac)), 0., 0.)]

    # Add qp_slack terms to objective
    obj += mycas.dot(pars["lam"].sx, qp_slack.sx)
    obj += mu * 0.5 * mycas.dot(qp_slack.sx, qp_slack.sx)

    constraints += [mycas.MyConstr(grid - qp_slack.sx, 0., 0., name="grid_constraint")]

    nlp_solver = mycas.MyNLPSolver(obj, states, constraints, pars, solver=qp_solver_name)
...
def _solve_second_order_problem(
    ...
    horizon = len(old_lam)
    nlp_solver = _create_second_order_dual_problem(horizon, sys_ids, mu, qp_solver_name, active_constraints)
...
def handle_slack_local_problem(signal:signal.SecondOrderSignal, rho, p_lim, qp_solver="osqp"):
    """
    ...
    obj += rho / 2. * mycas.dot(y.sx - pars["ref_grid_power_slack"].sx, y.sx - pars["ref_grid_power_slack"].sx)

    nlp_solver = mycas.MyNLPSolver(obj, states, constraints, pars, solver=qp_solver)
    nlp_solver.solve({
        "ref_grid_power_slack": signal.res_power_set,
        "lam_slack": signal.mul_lambda
    })
...
def handle_static_agent_problem(
    ...
    y_static = mycas.MySX("y_static", -np.infty, np.infty, horizon=horizon)
    states = [y_static]

    # Implicitly this is the constraint of the slack agent.
    constraints += [mycas.MyConstr(y_static.sx - uncontrolled_agents, -p_lim, p_lim, "static grid limit")]

    obj = mycas.dot(pars["lam_static"].sx, y_static.sx)
    obj += rho / 2. * mycas.dot(y_static.sx - pars["ref_grid_power_static"].sx, y_static.sx - pars["ref_grid_power_static"].sx)

    nlp_solver = mycas.MyNLPSolver(obj, states, constraints, pars, solver=qp_solver)
    nlp_solver.solve({
        "ref_grid_power_static": signal.res_power_set,
        "lam_static": signal.mul_lambda
    })
