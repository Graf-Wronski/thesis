...
from grecco_sim.coordinator import coordinator, second_order

from grecco_sim.util import helper, signals
from grecco_sim.util import type_defs


class CoordinatorADMM(coordinator.Coordinator):
    """
    ...
    def __init__(self, controller_pars: type_defs.OptParameters, sys_ids: list[str], grid: type_defs.GridDescription):
        ...
        self.ref_change = np.array([[np.infty]])
    ...
    def get_signal(self, sys_id) -> signal.SecondOrderSignal:
        ...
        if sys_id in self.ref_power_grid:
            ...
            return signals.FirstOrderSignal(np.zeros(1))
    ...
    def has_converged(self, time_index):
        ...
        if (
            abs(self.prev_constraint_value - self.constraint_value)
            < self.signal_tolerance
        ):
            print(
                f"Breaking iteration with constraint violation of {self.constraint_value}"
            )
            return True
    ...
    def __init_iteration(self, futures: Dict[str, type_defs.Schedule]):
        ...
        self.ref_power_grid[self.SLACK_AGENT] = -np.array([futures[a].yg for a in futures]).sum(axis=0)
    ...
    def get_signals(
        ...
        yk_slack = second_order.handle_static_agent_problem(
            signal.SecondOrderSignal(self.ref_power_grid[self.SLACK_AGENT], self.lam[self.SLACK_AGENT]),
            self.opt_pars.rho,
    ...
    def empty_signal(self) -> signals.Signal | None:
...
class ADMMCentralSolver(object):
    ...
    def _create_problem(self):
        ...
        for sys_id in self.sys_ids:
            ...
            ref_grid_a = mycas.MySX(f"ref_grid_a_{sys_id}", -np.infty, np.infty, horizon=self.horizon)
            states += [ref_grid_a]
            grid += ref_grid_a.sx

            obj += self.rho / 2. * mycas.dot(
        ...
        self.nlp_solver = mycas.MyNLPSolver(
            obj, states, constraints, self.pars, solver=self.qp_solver
        )
    ...
    def solve(self, lam, yg):
        ...
        ret_ref_grid = {
            a: self.nlp_solver.opt_vector(f"ref_grid_a_{a}")
            for a in lam
        }
        return ret_ref_grid
