...
class SolverThermalSystem(local_solver_base.LocalSolverBase):
    ...
    def __init__(
        self, horizon: int, sys_id: str, sys_pars: type_defs.SysParsHeatPump, opt_pars: type_defs.OptParameters
    ) -> None:
    ...
    def _create_problem(
        ...
        self._problem, self._grid_var = _get_therm_system_contributions(sys_id, horizon, sys_pars, opt_pars)

    def get_central_problem_contribution(self)-> tuple[tuple[mycas.casadi.SX, list[mycas.MySX], list[mycas.MyConstr], dict[str, mycas.MyPar]], mycas.MySX]:
        """Return the systems contributions to a combined OCP"""
        return self._problem, self._grid_var


def _get_therm_system_contributions(
    sys_id: str, horizon: int, sys_pars: type_defs.SysParsHeatPump, opt_pars: type_defs.OptParameters
    ...
    # House temperature
    temp = mycas.MySX(f"x_a_{sys_id}", -np.infty, np.infty, horizon=horizon + 1)
    states += [temp]

    # Introduce slack constraint to ensure feasible satisfaction of temperature constraints.
    slack_th = mycas.MySX(f"slack_th_a_{sys_id}", 0, np.infty, horizon=horizon + 1)
    states += [slack_th]
    constraints += [mycas.MyConstr(temp.sx + slack_th.sx, sys_pars.temp_min_heat, np.infty)]
    constraints += [mycas.MyConstr(-temp.sx + slack_th.sx, -sys_pars.temp_max_heat, np.infty)]
    ...
    p_norm = mycas.MySX(f"_p_norm_{sys_id}", 0., 1., horizon=horizon, discrete=True)
    states += [p_norm]

    p_el_hp = mycas.MySX(f"p_el_hp_{sys_id}", 0, sys_pars.p_max, horizon=horizon)
    states += [p_el_hp]
    constraints += [mycas.MyConstr(p_norm.sx * sys_pars.p_max - p_el_hp.sx, 0, 0.)]
    ...
    for k in range(horizon):
        ...
        constraints += [mycas.MyConstr(xs.sx[k] - xf.sx[k] - pars["fc_p_unc"].sx[k] - p_el_hp.sx[k], 0, 0)]
        ...
        q_net = (
            pars["solar_heat_gain"].sx[k] * sys_pars.absorbance * sys_pars.irradiance_area / 1000.
            + sys_pars.heat_rate * (pars["temp_outside"].sx[k] - temp.sx[k])
            + sys_pars.cop * p_el_hp.sx[k]
        )
        # Constrain state evolution
        x_plus = temp.sx[k] + q_net * sys_pars.dt_h * 3600. / sys_pars.thermal_mass
