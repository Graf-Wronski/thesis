...
from grecco_sim.local_problems import local_solver_base
...
class CasadiSolverLocalPVBatVectorized(local_solver_base.LocalSolverBase):
    ...
    def __init__(self, horizon: int, sys_id: str, sys_pars: SysParsPVBat, opt_pars: OptParameters) -> None:
        self.sys_pars = sys_pars
        self._create_problem(horizon, sys_id, sys_pars, opt_pars)

    def _create_problem(self, horizon: int, sys_id: str, sys_pars: SysParsPVBat, opt_pars: OptParameters) -> None:
        ...
        constraints += [mycas.MyConstr(x.sx[0] - pars["x_init"].sx, 0, 0)]
        ...
        # Feed and supply
        xf = mycas.MySX(f"xf_a_{sys_id}", 0, np.infty, horizon=horizon)
        states += [xf]
        xs = mycas.MySX(f"xs_a_{sys_id}", 0, np.infty, horizon=horizon)
        states += [xs]
        ...
        constraints += [mycas.MyConstr(xf.sx - pars["fc_pv_prod"].sx, -np.infty, 0)]

        # Grid variable
        y_g = mycas.MySX(f"y_g_a_{sys_id}", -np.infty, np.infty, horizon=horizon)
        states += [y_g]

        # Formulate the NLP
        for k in range(horizon):
            # Supply - Feed_in = Load - PV + discharge - charge
            constraints += [
                mycas.MyConstr(xs.sx[k] - xf.sx[k] - pars["fc_p_unc"].sx[k] + p_dch.sx[k] - p_ch.sx[k], 0, 0)
            ]

            if isinstance(sys_pars.c_sup, list):
                obj += xs.sx[k] * sys_pars.c_sup[k] - xf.sx[k] * sys_pars.c_feed
            else: 
                obj += xs.sx[k] * sys_pars.c_sup - xf.sx[k] * sys_pars.c_feed
            ...
            constraints += [mycas.MyConstr(x_plus - x.sx[k+1], 0., 0.)]

        # =================== Transform to casadi input ==============================
        self.nlp_solver = mycas.MyNLPSolver(obj, states, constraints, pars, solver=opt_pars.solver_name)
        self.problem = obj, states, constraints, pars
        self._grid_var = y_g
    ...
    def get_user_functions(self) -> dict[str, mycas.casadi.SX]:
        ...
        pars = self.problem[3]
        xf = states[3]
        user_funcs["xf_lb"] = xf.sx - pars["fc_pv_prod"].sx
...
class OnOffPVBatSolver(object):
    ...
    def _create_problem(self, pars):
        ...
        for k in range(self.horizon):
            ...
            constraints += [mycas.MyConstr(xsk.sx - xfk.sx - self.pars["fc_p_unc"].sx[k] +
                                           self.p_max_charging * (p_dch.sx - p_ch.sx), 0, 0)]
            ...
            # Constrain state evolution
            x_plus = list_xk[-2].sx + pars["dt_h"] / self.capacity * (self.p_max_charging * p_ch.sx * self.eff
                                                                      - self.p_max_charging * p_dch.sx / self.eff)
            # f_x_next(x0=list_xk[-2].sx, u=uk.sx)["x_next"]
    ...
    def solve(self, soc_init, fc_res_load, signal):
        ...
        ret_uk = (
            self.nlp_solver.opt_vars([f"p_ch_k_{k}_a_{self.sys_id}" for k in range(self.horizon)])
        ...
        if False:
            ret_x = self.nlp_solver.opt_vars([f"x_k_{k + 1}_a_{self.sys_id}" for k in range(self.horizon)])
            ret_xs = self.nlp_solver.opt_vars([f"xs_k_{k}_a_{self.sys_id}" for k in range(self.horizon)])
            ret_xf = self.nlp_solver.opt_vars([f"xf_k_{k}_a_{self.sys_id}" for k in range(self.horizon)])
