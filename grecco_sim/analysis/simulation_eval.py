...
def signal_analysis(
    ...
    for k, signal in list(raw_output.values())[0]["signals"].items():
        ...
        if not isinstance(signal, signal.SecondOrderSignal):
            print(f"Signal not Second order signal but {type(signal)}")
            return

        sig = ax_sig.plot(np.arange(start=k, stop=k+signals.Signal_len), signal.mul_lambda, label=f"k = {k}", drawstyle="steps-post")
        ax_sig.plot(k, signal.mul_lambda[0], marker="s", color=sig[0].get_color(), label=None)
        ref = ax_ref.plot(np.arange(start=k, stop=k+signals.Signal_len), signal.res_power_set, label=f"k = {k}",drawstyle="steps-post")
