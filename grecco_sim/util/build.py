...
def solver(
    ...
    if solver_name == "osqp":
        ...
        solver_options["gurobi"] = {"TimeLimit": 5, "OutputFlag": 0, "LogToConsole": 0}
...
def cummulative_ev_lims(
    ...
    requests = [
        r for r in request_list if r.start_step <= now + horizon and r.end_step >= now
    ]
...
def temporal_resolution(name: str) -> Callable[[np.ndarray], np.ndarray]:
    ...
    if name == "cubic":
        ...
        return lambda x: np.random.normal(loc=0.0, scale=0.33, size=x.shape)
